...
function projectChanged(projDropdown, row){
	...
	new Ajax.Updater('issue_container', issUrl,{
		method: 'get',
        parameters: closedIssueInd ? { project_id: id, user_id: uid, format:fmt, closed_issue_ind: closedIssueInd} :
		{ project_id: id, user_id: uid, format:fmt},
...
function reloadIssues(closedIssueInd){
	...
	if(project_ids.length > 0){
		...
		method: 'get',
        parameters: closedIssueInd ? { 'project_ids[]': project_ids, user_id: uid, closed_issue_ind: closedIssueInd, mon: mon, format:fmt} :
		{ 'project_ids[]': project_ids, user_id: uid, mon: mon, format:fmt},
...
function addRow(){
	...
	var row = issueTable.insertRow(rowCount-1);
	...
	renameElemProperties(row, 0, rowCount-1);
...
function calculateTotal(day){
	...
	//since it has a header and total row use <  rowCount-1 and i=1
